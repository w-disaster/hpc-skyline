...
int main(int argc, char* argv[]){
	...
	for(int k = 0; k < 10; k++){
		for(int i = 1; i <= 32; i++){
			...
			cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, skyline, 0, 1024);
			printf("min grid size: %d, block size: %d\n", minGridSize, blockSize);
	//		cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks, skyline, y_dim, 0);
	//		printf("num blocks per SM: %d\n", num_blocks);
			
			avg[i - 1] += t_kernel_end - t_kernel_start;
	//		printf("y dim: %d, time: %lf\n", y_dim, t_kernel_end - t_kernel_start);
			cudaCheckError();	
		}
	}
